...
/**
 * Recall cost at scale — the measurement behind the M5 trigger points in
 * docs/14-MEMORY.md §6, so "when do we need an ANN index?" is answered with
...
describe('recall cost at scale', () => {
  it('measures the per-query cost at 1k / 10k rows', () => {
    ...
    // runner, the M5 work (worker-thread scoring, ANN index) is overdue.
