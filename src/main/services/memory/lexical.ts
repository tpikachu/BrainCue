 ...
 * tickets. So recall runs both and unions the results (docs/14-MEMORY.md §3).
 *
 * Why not SQLite FTS5: the test harness (sql.js) ships FTS3/FTS4 but NOT FTS5,
 * so an FTS5 virtual table would fail every migration under test and leave this
 * path — which decides what the app says out loud — unverifiable. FTS4 has no
 * BM25 ranking, which is the only reason to want FTS in the first place.
 * Scoring here in JS is testable on every engine and gives direct control over
 * the rare-token weighting below. If corpus size ever makes this the
 * bottleneck, the swap point is this module's interface, not its callers.
 */
 ...
 */
export function lexicalScore(queryTokens: string[], content: string, index: LexicalIndex): number {
